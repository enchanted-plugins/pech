...
Audience: Claude. Nook is the cost ledger for AI-assisted development — attributes every token, tool-use turn, prompt-cache hit/write, and batch job to the plugin / sub-plugin / skill / agent tier / model that fired it. Forecasts spend, fires budget thresholds, surfaces cache waste, learns per-developer patterns.
...
Nook is **hook-driven**. The observation loop never asks permission — every tool call is measured silently. Skill-invocation is only for developer-facing queries (`/nook-cost`, `/nook-forecast`, `/nook-attribute`, `/nook-report`).
...
| `Stop` | cost-tracker | Finalize session rollup; publish `nook.session.cost.finalized` |
| `PreCompact` | nook-learning | Persist per-developer spend patterns (L5); export to `shared/learnings.json` |
| `/nook-cost [scope]` | cost-query | Display current spend (session by default; `--day`, `--month`) |
| `/nook-forecast [scope]` | cost-query | Display L1 forecast with confidence band |
| `/nook-attribute [tool]` | cost-query | Break down last N calls by plugin/tier/model |
| `/nook-report` | cost-query | Generate dark-themed PDF audit (Opus orchestrator for anomaly triage) |

See `./plugins/<name>/hooks/hooks.json` for matchers. Agents in `./plugins/<name>/agents/`.

## Algorithms

L1 Exponential Smoothing · L2 Budget Boundary Detection · L3 Z-Score Cost Anomaly · L4 Cache-Waste Measurement · L5 Gauss Learning (Nook). Derivations in `docs/science/README.md`. **Defining engine:** L1 — forecasting is what makes cost data actionable; raw ledgers are just bookkeeping.
...
| L5 | Gauss Learning (Nook) | nook-learning | Weighted moving averages over per-developer spend signals (per skill, per session type). Allay-A4 atomic serialization. |
...
2. **YOU MUST consume Allay's A2 token counts — never re-tokenize.** [A] A2 Linear Runway Forecasting owns token counting. Nook reads the API response's `usage` field (authoritative) and Allay's published events (for in-flight estimates). Re-tokenizing duplicates CPU and creates a second counter that will drift out of sync.
3. **YOU MUST NOT emit per-call events on the bus.** [H] Event-bus traffic scales with threshold crossings + rollups, never with call volume. At 10k calls/day per-call emission floods every subscriber. Enforced by the `nook_publish` helper in `shared/constants.sh` which rate-limits to one event per scope-window.
4. **YOU MUST attribute per agent tier, not per parent thread.** [A] A Flux `/converge` run fires Opus (~$0.06) → Sonnet loop (~$2.00) → Haiku validator (~$0.003). Naïve parent-thread attribution puts Sonnet under Opus's line, making Opus look 30× costlier. `ENCHANTED_ATTRIBUTION.agent_tier` is set at dispatch time.
5. **YOU MUST treat orphan spend as a bug signal, not a default bucket.** [A] A hook that fires a model call without setting `ENCHANTED_ATTRIBUTION` is broken. Surface orphan rate as a first-class health metric (`nook.attribution.orphan_rate.crossed` at > 1%). Do not quietly bucket orphans into a generic "other".
...
|------|---------|--------|--------------|
| Advisory (default) | Threshold at 50/80/100% | Emit `nook.budget.threshold.crossed`; peer plugins degrade (Flux → Haiku; Weaver → defer PR polish) | None needed — advisory only |
| Hard (opt-in via `NOOK_HARD_BUDGET=1`) | Threshold at 100% | Route to `weaver-gate` for dev confirmation to continue | `--yes-i-know-cost` flag for one invocation |
| Hybrid (opt-in via `NOOK_HYBRID_BUDGET=1`) | Advisory for Sonnet/Haiku; hard for Opus > $5/hour | Per-tier enforcement | Per-tier escape hatches |
...
| Tier | Model | Used for |
|---|---|---|
| Orchestrator | Opus | `/nook-report` anomaly triage ("why did this session spike?"); L3 anomaly narrative generation |
...
- **Re-tokenizing client-side.** Allay's A2 already does this; Anthropic's API returns authoritative `usage`. Two counters drift, CPU doubles, cost signal worsens.
...
- **Hard-block without developer opt-in.** Nook is showback by default. Surprise-blocking a `/converge` run mid-loop destroys trust; the developer owns the decision to enforce.
...
4. **Allay-style marketplace.** 5 sub-plugins + `full` meta, each shipping `.claude-plugin/plugin.json` + `{agents,commands,hooks,skills,state}/` + `README.md`.
5. **Dark-themed PDF report.** `/nook-report` produces it via `docs/architecture/generate.py` + puppeteer.
...
Events this plugin publishes: `nook.budget.threshold.crossed`, `nook.session.cost.finalized`, `nook.rate_card.refreshed`, `nook.rate_card.stale`, `nook.attribution.orphan_rate.crossed`, `nook.anomaly.detected`
Events this plugin subscribes to: `allay.api.usage.observed` (primary cost input), `allay.runway.threshold.crossed` (forecast correlation), `flux.prompt.crafted` (attribution context), `hornet.change.classified` (attribution context), `reaper.prepush.secret.detected` (attribution context), `weaver.task.boundary.detected` (attribution context), `weaver.commit.committed` (attribution context)
