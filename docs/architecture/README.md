...
- **`generate.py` raises KeyError**: a sub-plugin's `plugin.json` is missing a required field. Compare against any sibling (`enchanted-plugins/wixie/plugins/prompt-crafter/.claude-plugin/plugin.json`).
